-- Add service_types column to providers table
-- This will store an array of service types that each provider offers

ALTER TABLE providers 
ADD COLUMN IF NOT EXISTS service_types TEXT[];

-- Add a comment to explain the column
COMMENT ON COLUMN providers.service_types IS 'Array of service types this provider offers (e.g., Veterinary, Groomer, Trainer, etc.)';

-- Create an index for faster filtering by service types
CREATE INDEX IF NOT EXISTS idx_providers_service_types ON providers USING GIN (service_types);

-- Update the marketplace_listings view to include service_types
