-- Case Management System Migration (Fixed)
-- Creates tables and functions for veterinarian case tracking and workflow management
-- This version removes CHECK constraints with subqueries and uses triggers instead
...
CREATE TABLE IF NOT EXISTS cases (
    ...
    -- Relationships (foreign keys will be validated by triggers)
...
RETURNS TRIGGER AS $$
BEGIN
    -- Validate assigned nurse is a provider (skip validation if profiles table doesn't exist yet)
    ...
    -- Validate escalated vet is a provider (skip validation if profiles table doesn't exist yet)
